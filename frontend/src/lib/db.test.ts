...
const h = vi.hoisted(() => ({ state: { inserted: null as Record<string, unknown> | null } }));
...
vi.mock('./supabase', () => {
  const builder = () => ({
    select() { return this; },
    order() { return this; },
    limit() { return Promise.resolve({ data: [{ position: 2 }], error: null }); },
...
beforeEach(() => { h.state.inserted = null; });
