/* ============================================================
   Réelgram — Cinematic Player
   ============================================================ */

function PlayerScreen({ video, categories, onBack, onUpdate, onDelete }) {
  const [playing, setPlaying] = React.useState(true);
  const [progress, setProgress] = React.useState(0.32);
  const [sheet, setSheet] = React.useState(null); // 'rename' | 'category' | 'delete'
  ...
  return (
    <div className="view modal-enter" style={{ background: "#060608" }}>
      ...
      {/* bottom sheets */}
      {sheet && <PlayerSheet
        type={sheet} video={video} title={title} categories={categories} cat={cat}
...
function PlayerSheet({ type, title, categories, cat, onClose, onRename, onCategory, onDelete }) {
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--bg-1)", borderRadius: "26px 26px 0 0", border: "1px solid var(--hairline)", borderBottom: "none",
        ...
        {type === "more" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <SheetRow icon={Icons.edit} label="Renommer" onClick={() => onRename ? null : null} disabled />
...
function SheetRow({ icon: Icon, label, onClick, disabled }) {
  return (
