# Déploiement & développement — Réelgram

> Guide technique complet. Pour la présentation du produit, voir le
> [README](../README.md).
...
gitignoré — ne le committez jamais ; **un commentaire `# …` collé après une
valeur n'est PAS retiré** : mettez chaque valeur seule sur sa ligne).
...
Voir **[`ios-shortcut.md`](ios-shortcut.md)** : installer la PWA sur l'écran
d'accueil et créer un Raccourci iOS pour envoyer un Reel vers le vault depuis la
feuille de partage Instagram (authentifié par un token personnel généré dans
l'app).
